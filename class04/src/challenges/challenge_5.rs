...
fn is_constructable(str1: String, str2: String) -> bool {
    ...
    let map1 = str1.chars().fold(HashMap::new(), |mut map, char| {
        *map.entry(char).or_insert(0) += 1;
        map
    });
    let map2 = str2.chars().fold(HashMap::new(), |mut map, char| {
        *map.entry(char).or_insert(0) += 1;
        map
    });

    map1.iter()
        .all(|(char, count)| map2.get(char).map_or(false, |count2| count2 >= count))
}
...
mod test {
    ...
    fn is_constructable_1() {
        let data = ("a", "b", false);
        assert_eq!(
            is_constructable(data.0.to_owned(), data.1.to_owned()),
            data.2
        )
    }
    #[test]
