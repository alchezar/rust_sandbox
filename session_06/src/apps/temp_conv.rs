...
impl Temp {
	fn new(value: f32, from_type: TempType, converted: bool) -> Self {
		Self { value, from_type, converted }
	}
	#[must_use]
	fn get_info() -> Self {
		loop {
	...
	fn get_value() -> Option<Self> {
		...
		std::io::stdin()
			.read_line(&mut temp_type_input)
			.ok()?;
		let from_type = match temp_type_input
			.trim()
			.parse::<u8>()
			.ok()?
		{
		...
		std::io::stdin()
			.read_line(&mut temp_value_input)
			.ok()?;
		let value = temp_value_input
			.trim()
			.parse::<f32>()
			.ok()?;
