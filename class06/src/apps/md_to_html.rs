...
fn convert_parallel(files: Vec<path::PathBuf>, sender: mpsc::Sender<ProcessorMessage>) -> Vec<thread::JoinHandle<()>> {
	files
		.into_iter()
		.enumerate()
		.map(|(index, file)| {
			...
			thread::spawn(move || {
				let contents = match fs::read_to_string(&file) {
					...
					Err(error) => {
						sender
							.send(ProcessorMessage::Error(format!("Failed to read file : {error}")))
							.unwrap();
						return;
					}
				};
				...
				sender
					.send(ProcessorMessage::Success(format!("File {file:?} is processed")))
