...
async fn uploader(
    ...
    if let (Some(tags), Some(image)) = (tags, image) {
        let new_image_id = insert_image_into_db(&pool, &tags).await.unwrap();
    ...
    tokio::fs::read_to_string(&path).await.unwrap().into()
}
...
fn make_thumbnail(id: i64) -> anyhow::Result<()> {
    ...
    image.thumbnail(64, 64).save(thumbnail_path)?;
...
async fn get_thumbnail(
    axum_path::Path(id): axum_path::Path<i64>,
...
async fn search_images(
    ...
    let rows =
        sqlx::query_as::<_, ImageRecord>("SELECT * FROM images WHERE tags LIKE ? ORDER BY id")
            .bind(tag)
            .fetch_all(&pool)
            .await
            .unwrap();
