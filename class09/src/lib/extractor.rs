...
pub mod example_2 {
    ...
    async fn main() -> std::io::Result<()> {
        HttpServer::new(|| App::new().service(index_1).service(index_2))
            .bind("localhost:8080")?
            .run()
            .await
    }
    ...
    async fn index_3(req: HttpRequest) -> std::io::Result<String> {
        let name = req
            ...
            .parse::<String>()
            .unwrap();
        let user_id = req.match_info().query("user_id").parse::<i32>().unwrap();
...
pub mod example_6 {
    ...
    async fn show_count(data: web::Data<AppState>) -> impl Responder {
        ...
        format!("count: {}", data.global_count.load(Ordering::Acquire))
    }

    async fn add_one(data: web::Data<AppState>) -> impl Responder {
        // *data.count.write().await += 1;
        // format!("count: {}", data.count.read().await)
        data.global_count.fetch_add(1, Ordering::AcqRel);
        format!("count: {}", data.global_count.load(Ordering::Acquire))
    }
